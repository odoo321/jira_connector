...
class JiraProjectVersion(models.Model):
    ...
    def send_to_jira(self):
        ...
        if not self.jira_id:
            response = self.env.ref('jira_connector.jira_settings_record').post('version', version_dict)
            self.jira_id = response.json()['id']
        else:
            response = self.env.ref('jira_connector.jira_settings_record').put('version/' + self.jira_id, version_dict)
    ...
    def unlink(self):
        ...
        if jira_id:
            self.env.ref('jira_connector.jira_settings_record').delete('version/' + jira_id)
        return output

    def jira_get_all(self):
        for version in self.search([]):
            self.jira_parse_response(
                self.env.ref('jira.connector_jira_settings_record').get('version/' + version.jira_id).json()
            )
    ...
    def jira_key(self, id):
        ...
        if not version:
            version = self.jira_parse_response(
                self.env.ref('jira_connector.jira_settings_record').get('version/' + id).json()
            )
        return version
