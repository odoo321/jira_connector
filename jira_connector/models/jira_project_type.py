...
class JiraProjectType(models.Model):
    ...
    def jira_get_all(self):
        response = self.env.ref('jira_connector.jira_settings_record').get('project/type').json()
        for pt in response:
    ...
    def jira_key(self, key):
        ...
        if not project_type:
            response = self.env.ref('jira_connector.jira_settings_record').get('project/type/' + key).json()
