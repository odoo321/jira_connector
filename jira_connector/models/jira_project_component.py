...
class JiraProjectComponent(models.Model):
    ...
    def send_to_jira(self):
        ...
        if not self.jira_id:
            response = self.env.ref('jira_connector.jira_settings_record').post('component', component_dict)
            self.jira_id = response.json()['id']
        else:
            response = self.env.ref('jira_connector.jira_settings_record').put('component/' + self.jira_id, component_dict)
    ...
    def unlink(self):
        ...
        if jira_id:
            self.env.ref('jira_connector.jira_settings_record').delete('component/' + jira_id)
        return output

    def jira_get_all(self):
        for component in self.search([]):
            self.jira_parse_response(
                self.env.ref('jira_connector.jira_settings_record').get('component/' + component.jira_id).json()
            )
    ...
    def jira_key(self, id):
        ...
        if not component:
            component = self.jira_parse_response(
                self.env.ref('jira_connector.jira_settings_record').get('component/' + str(id)).json()
            )
        return component
