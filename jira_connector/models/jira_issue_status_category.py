...
class JiraIssueStatusCategory(models.Model):
    ...
    def jira_get_all(self):
        scategory = self.env.ref('jira_connector.jira_settings_record').get('statuscategory').json()
        for sc in scategory:
    ...
    def jira_key(self, key):
        ...
        if not sc:
            sc = self.jira_parse_response(
                self.env.ref('jira_connector.jira_settings_record').get('statuscategory/' + key).json()
            )
        return sc
