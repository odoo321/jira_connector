...
class JiraIssueResolution(models.Model):
    ...
    def jira_get_all(self):
        resolutions = self.env.ref('jira_connector.jira_settings_record').get('resolution').json()
    ...
    def jira_key(self, id):
        ...
        if not resolution:
            resolution = self.jira_parse_response(
                self.env.ref('jira_connector.jira_settings_record').get('resolution/' + id).json()
            )
        return resolution
