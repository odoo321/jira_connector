...
class JiraProjectCategory(models.Model):
    ...
    def jira_get_all(self):
        for pc in self.env.ref('jira_connector.jira_settings_record').get('projectCategory').json():
    ...
    def jira_key(self, id):
        ...
        if not category:
            category = self.jira_parse_response(
                self.env.ref('jira_connector.jira_settings_record').get('projectCategory/' + str(id)).json()
            )
        return category
