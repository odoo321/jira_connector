...
class JiraIssueType(models.Model):
    ...
    def jira_get_all(self):
        types = self.env.ref('jira_connector.jira_settings_record').get('issuetype').json()
        for t in types:
    ...
    def jira_key(self, id):
        ...
        if not t:
            t = self.jira_parse_response(
                self.env.ref('jira_connector.jira_settings_record').get('issuetype/' + id).json()
            )
        return t
